...
//    case updateUserDataWithImage(params: [String: Any], imageFiles: [(Data, String)]?, token: String)
    ...
    var method: HTTPMethod {
        switch self {
        case .getWelcome, .getConstants, .getUserProfile, .getConstantDetails, .map, .getOrders, .getOrderDetails, .getNotifications, .getWallet, .getCategories, .getSubCategories, .getAddressByType, .getAddressList, .getRates, .getAppConstants, .getHome, .guest, .getContact:
            return .get
        case .register, .verify, .resend, .updateUserDataWithImage, .updateUserData, .logout, .addOrder, .addOfferToOrder, .updateOfferStatus, .updateOrderStatus, .addReview, .deleteNotification, .addBalanceToWallet, .addComplain, .createReferal, .checkCoupon, .addAddress, .updateAddress, .deleteAddress, .getTotalPrices, .deleteAccount, .tamaraCheckout, .checkPlace, .checkPoint, .rechangePoint:
            return .post
        }
    }
    ...
    var headers: HTTPHeaders {
        switch self {
        case .getWelcome, .getConstants, .getConstantDetails, .register, .verify, .resend, .getCategories, .getSubCategories, .getAppConstants, .getHome, .guest, .getContact:
            var headers = HTTPHeaders()
            headers.add(name: "Accept-Language", value: getUserPreferredLanguageCode() ?? "ar")
            return headers
        case .getUserProfile(let token), .updateUserDataWithImage(_, _, let token), .updateUserData(_, let token), .logout(_, let token), .addOrder(_, let token), .map(_, let token), .addOfferToOrder(_, _, let token), .updateOfferStatus(_, _, let token), .updateOrderStatus(_, _, let token), .getOrders(_, _, _, token: let token), .getOrderDetails(_, let token), .addReview(_, _, let token), .getNotifications(_, _, let token), .deleteNotification(_, let token), .getWallet(_, _, let token), .addBalanceToWallet(_, let token), .addComplain(_ , let token), .createReferal(let token), .checkCoupon(_, let token), .addAddress(_, let token), .updateAddress(_, let token), .deleteAddress(_, let token), .getAddressByType(_, let token), .getAddressList(let token), .getTotalPrices(_, let token), .getRates(_, _, _, let token), .deleteAccount(_, let token), .tamaraCheckout(_, let token), .checkPlace(_, let token), .checkPoint(_, let token), .rechangePoint(_, let token):
            var headers = HTTPHeaders()
            headers.add(name: "Accept-Language", value: getUserPreferredLanguageCode() ?? "ar")
    ...
    var parameters: [String: Any]? {
        switch self {
        case .getWelcome, .getConstants, .getConstantDetails, .getUserProfile, .logout, .map, .getOrders, .getOrderDetails, .getNotifications, .deleteNotification, .getWallet, .createReferal, .getCategories, .getSubCategories, .getAddressByType, .getAddressList, .getRates, .getAppConstants, .getHome, .guest, .deleteAccount, .getContact:
            return nil
